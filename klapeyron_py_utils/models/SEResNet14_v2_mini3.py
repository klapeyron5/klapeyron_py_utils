...
class SEResNet14_v2_mini3(Base):
    ...
    NAME = 'SEResNet14_v2_mini3'

    def __init__(self, model_config: Model_Config, train_config: Model_Train_Config):
        super(SEResNet14_v2_mini3, self).__init__(model_config, train_config)

        self.input_shape = model_config.input_shape

        self.conv0 = Layer_conv_bn_relu((7, 7, self.input_shape[-1], 8), 2)

        Ns = [2, 2, 2]
        blocks_output_depths = [8, 16, 32]
        se_reduction = 4
...
def ut_0():
    ...
    sh = (1, size, size, 3)
    x = tf.zeros(sh, tf.float32)
    x1 = tf.ones(sh, tf.float32)
    xr = tf.random.truncated_normal(sh, stddev=0.5, dtype=tf.float32)
    r = SEResNet14_v2_mini3(m_config, m_trn_config)

    a = r.get_logits(x, False)
    a = r.get_logits(x, True)
    pass
